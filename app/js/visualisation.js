d3.dsv(";", "./src/data/loto_201911.csv").then(function (data) {
  ...
  data = data.filter(function (d) {
    return d.nombre_de_gagnant_au_rang1 != 0;
  });
  const jours = d3.rollup(
    data,
    (v) => d3.sum(v, (d) => d.nombre_de_gagnant_au_rang1),
    (d) => d.jour_de_tirage
  );
  console.log(jours);
  ...
  const scaleY2 = d3.scaleLinear().domain([0, 100]).range([height, 0]);
  ...
  const numeros = d3.rollup(
    data.flatMap((d) =>
      [d.boule_1, d.boule_2, d.boule_3, d.boule_4, d.boule_5].map(Number)
    ),
    (v) => v.length,
    (d) => d
  );
  ...
  const scaleY3 = d3.scaleLinear().domain([0, 30]).range([height, 0]);
...
function getGraphNumber(number, divId) {
  d3.dsv(";", "./src/data/loto_201911.csv").then(function (data) {
    ...
    function getAngle(number) {
      const count = d3.rollup(data, (v) =>
        d3.sum(v, (d) => {
          return [d.boule_1, d.boule_2, d.boule_3, d.boule_4, d.boule_5].filter(
            (b) => +b === number
          ).length;
        })
      );
    ...
    const svg4 = graph.append("svg").attr("viewBox", [0, 0, width, height]);
